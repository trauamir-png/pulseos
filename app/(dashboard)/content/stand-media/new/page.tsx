...
export default async function NewStandMediaPage({ searchParams }: { searchParams: Promise<DashboardSearchParams> }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-[var(--foreground)]">New video</h1>
        <p className="text-sm text-[var(--muted)]">{site.name}</p>
      </div>
      <StandMediaForm siteId={site.id} query={query} item={null} />
    </div>
  );
}
