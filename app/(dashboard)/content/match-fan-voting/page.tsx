...
import { getMatchFanPollsForSite } from "@/lib/dashboard/content-match-fan-voting";
import { MatchFanPollTable } from "@/components/content/match-fan-poll-list";
...
export default async function MatchFanVotingPage({ searchParams }: { searchParams: Promise<DashboardSearchParams> }) {
  ...
  if (!(await hasPermission(supabase, site.id, PERMISSIONS.CONTENT_MATCH_VOTING_VIEW))) {
    return <AccessDenied />;
  }
  const canManage = await hasPermission(supabase, site.id, PERMISSIONS.CONTENT_MATCH_VOTING_MANAGE);

  const items = await getMatchFanPollsForSite(supabase, site.id);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-[var(--foreground)]">Fan Match Voting</h1>
          <p className="text-sm text-[var(--muted)]">{site.name}</p>
        </div>
        {canManage && (
          <Link
            href={`/content/match-fan-voting/new${query}`}
      ...
      <MatchFanPollTable items={items} query={query} timeZone={site.timezone} />
    </div>
  );
}
