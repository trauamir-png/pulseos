...
import { getStandMediaForSite } from "@/lib/dashboard/content-stand-media";
import { StandMediaTable } from "@/components/content/stand-media-list";
...
export default async function StandMediaPage({ searchParams }: { searchParams: Promise<DashboardSearchParams> }) {
  ...
  if (!(await hasPermission(supabase, site.id, PERMISSIONS.CONTENT_STAND_MEDIA_VIEW))) {
    return <AccessDenied />;
  }
  const canManage = await hasPermission(supabase, site.id, PERMISSIONS.CONTENT_STAND_MEDIA_MANAGE);

  const items = await getStandMediaForSite(supabase, site.id);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-[var(--foreground)]">Stand Media</h1>
          <p className="text-sm text-[var(--muted)]">{site.name}</p>
        </div>
        {canManage && (
          <Link
            href={`/content/stand-media/new${query}`}
      ...
      <StandMediaTable items={items} query={query} timeZone={site.timezone} />
    </div>
  );
}
