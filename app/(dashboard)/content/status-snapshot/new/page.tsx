...
export default async function NewStatusSnapshotPage({ searchParams }: { searchParams: Promise<DashboardSearchParams> }) {
  ...
  return (
    <div dir="rtl" className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-[var(--foreground)]">עדכון חדש</h1>
        <p className="text-sm text-[var(--muted)]">{site.name}</p>
      </div>
      <StatusSnapshotForm siteId={site.id} query={query} item={null} />
    </div>
  );
}
